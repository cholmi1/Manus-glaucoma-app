...
function ClinicianDashboard({ role, actorId }: { role: "physician" | "admin"; actorId: string }) {
  const utils = trpc.useUtils();
  ...
  return <div className="space-y-6">
    ...
    {role === "admin" && <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><div className="flex items-center gap-3"><div className="grid h-11 w-11 place-items-center rounded-2xl bg-sky-100 text-sky-800"><Users className="h-5 w-5" /></div><div><p className="text-sm font-bold text-sky-700">관리자 권한</p><h2 className="text-xl font-extrabold">기관 사용자 역할 관리</h2></div></div><div className="mt-5 divide-y divide-slate-100">{membersQuery.data?.map(member => <div className="flex flex-col gap-3 py-4 sm:flex-row sm:items-center sm:justify-between" key={member.id}><div><p className="font-bold">{member.name || "이름 미설정"}</p><p className="mt-1 text-xs text-slate-500">{member.email || member.id}</p></div><select aria-label={`${member.name || "사용자"} 역할`} value={member.role} disabled={member.id === actorId} onChange={event => setRole.mutate({ userId: member.id, role: event.target.value as Role })} className="min-h-11 rounded-xl border border-slate-300 bg-white px-3 text-sm font-bold disabled:cursor-not-allowed disabled:opacity-60 focus:outline-none focus-visible:ring-4 focus-visible:ring-sky-200"><option value="patient">patient</option><option value="physician">physician</option><option value="educator">educator</option><option value="admin">admin</option></select></div>) ?? <p className="py-6 text-sm text-slate-500">사용자 목록을 불러오는 중입니다.</p>}</div></section>}
  </div>;
}
...
export default function Home() {
  ...
  if (!isAuthenticated || !user) return <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,_#e0f2fe_0,_#f8fafc_42%,_#eff6ff_100%)] px-5 py-6 sm:px-10"><div className="mx-auto flex min-h-[calc(100vh-3rem)] max-w-6xl flex-col justify-between"><header className="flex items-center gap-3"><div className="grid h-11 w-11 place-items-center rounded-2xl bg-sky-700 text-white"><Eye className="h-6 w-6" /></div><div><p className="text-lg font-extrabold">안압케어</p><p className="text-xs text-slate-500">안압 관리 의료 플랫폼</p></div></header><section className="grid gap-10 py-16 lg:grid-cols-[1.15fr_.85fr] lg:items-center"><div><p className="inline-flex items-center gap-2 rounded-full border border-sky-200 bg-white/80 px-3 py-1.5 text-xs font-bold text-sky-800"><ShieldCheck className="h-3.5 w-3.5" />Supabase Auth · 역할 기반 접근</p><h1 className="mt-6 text-4xl font-black tracking-[-.04em] text-slate-950 sm:text-6xl">환자 관리의<br /><span className="text-sky-700">신뢰 기반</span>을 만듭니다.</h1><p className="mt-6 max-w-xl text-base leading-7 text-slate-600">patient, physician, educator, admin이 각자의 역할에 맞춰 안압·점안·기기·알림 업무를 안전하게 관리합니다.</p><button type="button" onClick={() => void startLogin()} className="mt-8 inline-flex min-h-12 items-center gap-2 rounded-2xl bg-slate-950 px-6 text-sm font-extrabold text-white hover:bg-slate-800 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-sky-300">이메일 로그인 링크 받기 <ArrowRight className="h-4 w-4" /></button><p className="mt-3 text-xs text-slate-500">이메일을 입력하면 Supabase Auth 매직링크를 보냅니다.</p></div><div className="grid gap-4"><Stat label="환자 홈" value="1개" description="오늘의 우선 행동" /><Stat label="의료진 업무" value="3단계" description="즉시 조치 · 오늘 예정 · 모니터링" tone="rose" /><Stat label="보안 기준" value="RBAC" description="서버 측 역할 권한 검사" tone="slate" /></div></section><footer className="border-t border-slate-200 py-6 text-xs text-slate-500">의료 데이터는 역할별 서버 권한 검사와 감사 로그 해시 체인 기준으로 처리됩니다.</footer></div></main>;
